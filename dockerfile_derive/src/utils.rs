...
pub(crate) fn inner_type<'a>(wrapper_ty: &str, ty: &'a syn::Type) -> std::option::Option<&'a syn::Type> {
...
pub(crate) fn get_each_attr(attr: &Vec<syn::Attribute>, struct_ident: &syn::Ident) -> Result<syn::Ident, proc_macro2::TokenStream> {
    if attr.len() != 1 {
...
pub(crate) fn get_attr(attr: &Vec<syn::Attribute>, struct_ident: &syn::Ident) -> Result<AttrData, proc_macro2::TokenStream> {
    if attr.is_empty() {
...
pub(crate) fn verify_attr_ident<T: quote::ToTokens>(token: Option<TokenTree>, expected_ident: &str, span: T, err_msg: &str) -> Result<(), proc_macro2::TokenStream> {
    match token {
...
pub(crate) fn verify_attr_punct<T: quote::ToTokens>(token: Option<TokenTree>, expected_punct: char, span: T, err_msg: &str) -> Result<(), proc_macro2::TokenStream> {
    match token {
